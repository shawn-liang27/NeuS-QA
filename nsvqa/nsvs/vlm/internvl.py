...
class InternVL:
    ...
    def parallel_preprocess_video(self, frames, num_workers=8):
        """Processes entire video frames in parallel on CPU."""
        max_patches = self.max_patches
        # with ProcessPoolExecutor(max_workers=num_workers) as executor:
        #     func = partial(_single_frame_worker, max_num=max_patches, input_size=448)
        #     results = list(tqdm(executor.map(func, frames), total=len(frames), desc="Preprocessing Frames"))
        results = []
        for f in tqdm(frames):
            results.append(_single_frame_worker(f, max_patches, 448))
        
        # Flatten results
        all_pixels = torch.cat([r[0] for r in results], dim=0)
        all_patches = [r[1] for r in results]
        return all_pixels, all_patches
